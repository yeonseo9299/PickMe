...
- 카테고리별 랜덤 결정
- 비로그인 상태의 간단한 선택지 등록 / 결정(브라우저 localStorage)
...
- `/register` : 회원가입
- `/login` : 로그인
- `/choices/register` : 선택지 등록 (비로그인 가능)
- `/choices` : 선택지 관리
- `/decision` : 카테고리별 랜덤 결정 (비로그인 가능)
- `/history` : 결정 기록

선택지 관리와 결정 기록은 로그인 후 이용합니다. 로그인하지 않은 사용자는 선택지 등록과 결정만 간단하게 이용할 수 있으며, 이때 선택지는 브라우저 localStorage에 임시 저장되고 결정 기록은 저장하지 않습니다.
